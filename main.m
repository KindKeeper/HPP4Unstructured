...
%% 算法配置模块
config.global_planner = 'dstar';      % 可选: astar/dijkstra/dstar/rrt
config.local_planner = 'apf';         % 可选: dwa/apf
config.replan_strategy = 'dynamic';   % 重规划策略: dynamic/static
config.map_size = [100, 100];           % 新增：地图尺寸 [行,列]
...
if blocked
    ...
    refined_path2 = stitch_paths(refined_path1, replanned_path, blockage_index);
else
    refined_path2 = [];
end
